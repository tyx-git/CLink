...
#ifdef _WIN32
#include "src/server/modules/process_inject/include/process_injector.hpp"
...
bool ProcessManager::start(const clink::core::config::Configuration& config) {
    ...
    try {
        ...
        ipc_server_->set_packet_handler([this, session_state](
            ...
            } else if (header.type == clink::hook::ipc::PacketType::DataRecv) {
                session_state->packets_total.fetch_add(1, std::memory_order_relaxed);
                if (logger_) {
                    logger_->debug("IPC DataRecv ignored from injected client sid len", header.socket_id, body.size());
                }
