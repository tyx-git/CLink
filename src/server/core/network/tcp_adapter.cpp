...
namespace {
bool parse_endpoint_host_port(const std::string& endpoint, std::string& host, int& port) {
    if (endpoint.empty()) {
        return false;
    }
    ...
    if (endpoint.front() == '[') {
        ...
        port = std::stoi(endpoint.substr(right_bracket + 2));
        return true;
    }
...
std::error_code TcpTransportAdapter::start(const std::string& endpoint) {
    ...
    std::string ip;
    int port = 0;
    if (!parse_endpoint_host_port(endpoint, ip, port)) {
