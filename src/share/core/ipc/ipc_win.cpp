...
private:
    ...
    void process_client(HANDLE hPipe) {
        std::string raw;
        if (!read_frame(hPipe, raw)) return;
        ...
        if (write_frame(hPipe, out)) {
            ...
            logger_->error("[ipc.req.end] id=" + std::to_string(request_id) +
    ...
    void signal_shutdown() {
        ...
        if (hClient != INVALID_HANDLE_VALUE) {
            std::string shutdown_frame = detail::build_wire_message(kShutdownCommand, "");
