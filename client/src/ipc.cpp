        ...
        auto send_once = [&](bool allow_retry_hint) -> Message {
            ...
            while (remaining > 0) {
                if (!WriteFile(hPipe, write_ptr, static_cast<DWORD>(remaining), &bytesWritten, NULL)) {
                    ...
                    return {MessageType::Response,
                            request.command,
                            make_error_payload("command=" + request.command + " failed to write to pipe (error " + std::to_string(err) + ")")};
                }
            ...
            do {
                ...
                if (err == ERROR_BROKEN_PIPE) {
                    ...
                    resp.payload = make_error_payload("command=" + request.command + " failed to read from pipe (error " + std::to_string(err) + ")");
                }
